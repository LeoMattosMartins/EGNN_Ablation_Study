...
class EGNN(nn.Module):
    ...
    ABLATION: No distance information in messages."""
    ...
    def __call__(self, node_feat, pos, vel, edges):
        ...
        for _ in range(N_LAYERS):
            ...
            # dist_sq calculation remains conceptually, but removed from messages

            # --- Messages from sender features, receiver features (and distances REMOVED) ---
            m_ij = nn.relu(
                nn.Dense(D_MSG)(h)[:, :, None, :]  # sender:   h_i → [batch, N, 1, D]
                + nn.Dense(D_MSG)(h)[:, None, :, :]  # receiver: h_j → [batch, 1, N, D]
                # EXCLUDED: + nn.Dense(D_MSG)(dist_sq)
            )
